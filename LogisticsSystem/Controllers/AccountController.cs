    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            try
            {
                ...
                //bool user = UnitOfWork.UserRepository.Get(x => x.Email == model.Email && x.Password == model.Password).EmailConfirmed;              
                //if (user != false)
                //{
                    bool status = UnitOfWork.UserRepository.Get(x => x.Email == model.Email).IsActive;
                    if (status == true)
                    {
                    ...
                    {
                        Response.Write("<script>alert('User Not Approved. Please Wait for Admin Approval.')</script>");
                        return View();
                    }
                
                //else
                //{
                //    return View("ConfirmEmailFirst");
                //}
            }
            catch {
                ModelState.AddModelError("", "Invalid login attempt.");
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = new UM_User { UserName = model.Email, Email = model.Email };
